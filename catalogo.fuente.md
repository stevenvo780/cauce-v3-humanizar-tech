# Catálogo Mouseîon — Steven Vallejo (enriquecido, 2026-08-30 22:40Z)

Catálogo público de productos del ecosistema Mouseîon (stevenvallejo.com). Cada producto tiene metadata medida en vivo (HTTP, HTML, Vercel, GitHub).

## Leyenda de estado
- ✅ **Live** = HTTP 200, página carga
- 🟡 **Stale** = HTTP 200 pero sin cambios en 30+ días
- 🔴 **Broken** = HTTP 4xx/5xx, requiere fix
- ⚪ **Private/unknown** = producto existe en Vercel pero no accesible públicamente

## Productos Mouseîon — tuyos, públicos

| Producto | URL | HTTP | Stack | Idioma | Repo GitHub | Último push | Vercel project | Notas |
|---|---|---|---|---|---|---|---|---|
| **Mouseîon Portal** | [stevenvallejo.com](https://stevenvallejo.com) | 200 (110 KB) | Next.js | en | `mi-cv` | 07-ago 22:32 | `mi-cv` | CV + portfolio bilingüe. Full metadata, theme `#0b1417`. og:image auto-generada. |
| **Catálogo** | [catalogo.humanizar.tech](https://catalogo.humanizar.tech) | 200 (12.9 KB) | Next.js | es | `cauce-v3-humanizar-tech` | 13-ago 23:07 | `humanizar-catalogo` | theme `#0a0d12`. |
| **Agenda demos** | [agenda.humanizar.tech](https://agenda.humanizar.tech) | 200 (5.5 KB) | Next.js | es | `cauce-v3-humanizar-tech` | 13-ago 23:07 | `web-humanizar-agenda` | Form de booking. |
| **Cauce V3 (root)** | [humanizar.tech](https://humanizar.tech) | 200 | Next.js | es | `cauce-v3-humanizar-tech` | 13-ago 23:07 | `cauce-v3-humanizar-tech` | "Empresas de desarrollo hechas con agentes". theme `#0B1020`. |
| **Humanizar web** | [www.humanizar.co](https://www.humanizar.co) | 200 | Vite | es | `Humanizar` | 18-ago 15:14 | `humanizar` | Sitio corporativo. |
| **Nómos** | [nomos.stevenvallejo.com](https://nomos.stevenvallejo.com) | 🔴 500 | FastAPI+Neon | es | `scrapekit` | 10-ago 23:54 | `scrapekit` | 🔴 **Neon quota exhausted**. Issue abierta [scrapekit#1](https://github.com/stevenvo780/scrapekit/issues/1). |
| **Koinonía** | [koinonia.stevenvallejo.com](https://koinonia.stevenvallejo.com) | 200 (22 KB) | Next.js | es | `koinonia` | **HOY 16:28** | `communityos` | PWA con manifest. theme `#43b5a6`. Activa. |
| **Preontología** | [preontologia.stevenvallejo.com](https://preontologia.stevenvallejo.com) | 200 (1.7 KB) | HTML estático | es | — | — | `estructuras-preontologicas` | Tesis doctoral. theme `#0e1219`. |
| **Elenxos** | [www.elenxos.com](https://www.elenxos.com) | 200 (3.5 KB) | Next.js | es | `elenxos` | 20-abr (STALE) | — | "Software Académico y Sistemas Complejos". Apex `elenxos.com` falla (000). |
| **Agora (Elenxos)** | [agora.elenxos.com](https://agora.elenxos.com) | 200 (27 KB) | Next.js | es | — | — | `visormarkdown` | "Plataforma de investigación cooperativa". theme `#0a0a0f`. |
| **Apothḗke** | [apotheke.stevenvallejo.com](https://apotheke.stevenvallejo.com) | 200 (5.7 KB) | Next.js | es | `warehouse` (privado) | 07-ago 22:37 | `warehouse-app` | Gestión de almacén e inventario. og:image + PWA. |
| **Servicios Oscar** | [servicios.stevenvallejo.com](https://servicios.stevenvallejo.com) | 200 (43 KB) | Next.js | es-CO | `landing-servicios-oscar` | — | `landing-servicios-oscar` | "Agentes de IA que operan tu negocio". 5 servicios. theme `#0B1020`. |
| **Sinergia POS** | [sinergia-pos.vercel.app](https://sinergia-pos.vercel.app) | 🟡 200 (23h) | Next.js | es | `pos-back` | 29-ago 22:32 | `sinergia-pos` | POS para tiendas. |
| **Agentes IA Novacode** | [agentes.elenxos.com](https://agentes.elenxos.com) | 🟡 200 (23d) | Next.js | es | `novacode-agentes-ia` | 23-ago | `novacode-agentes-ia` | Marketing site de Novacode. |
| **Natación** | [natacion-asistencia.vercel.app](https://natacion-asistencia.vercel.app) | 🟡 200 (1d) | Next.js | es | — | — | `natacion-asistencia` | App de asistencia. |
| **SIGRE-CO** | [sigre.elenxos.com](https://sigre.elenxos.com) | 🟡 200 | Next.js | es | `sigre-co` | 15-ago 15:45 | `sigre-co` | Spec + Fase 0. Spec only, NO build. |
| **Prometeo** | (interno, en repo) | — | Next.js | — | `prometeo-b2b-sales` | **HOY 18:27** | — | "Agente de ventas B2B". Repo activo. |
| **Aletheia** | [aletheia.humanizar.tech](https://aletheia.humanizar.tech) | 🟡 200 | Next.js | es | `aletheia` | 20-ago 04:15 | `web-humanizar-mine` | "Plataforma de inteligencia de marketing". |
| **Devkits CRM (Xenia)** | [xenia.stevenvallejo.com](https://xenia.stevenvallejo.com) | ✅ 200 | Next.js | es | `devkits-crm` | activo | `devkits-crm` | CRM multi-tenant. |
| **Demeter** | [demeter.humanizar.cloud](https://demeter.humanizar.cloud) | ✅ 200 (3h) | Next.js | es | — | — | `dashboard` | Dashboard analítico. |
| **Graf Admin** | [admin.graf.com.co](https://admin.graf.com.co) | 🟡 200 (4d) | Next.js | es | `graf-client` | **HOY 17:16** | `graf-admin` | CRM admin. Repo activo hoy. |

## Productos de clientes (NO Mouseîon)

Estos viven en la misma cuenta de Vercel pero son de clientes — no los cuento como Mouseîon:

| Cliente | Producto | URL | Repo |
|---|---|---|---|
| Jhon | SIGRE-CO | sigre.elenxos.com | sigre-co (staging) |
| (varios) | devkits-hours | devkits-hours.vercel.app | — |
| (varios) | pulpos-marketing | pulso-marca-ia.vercel.app | — |

## Caídos / urgencia

| Producto | URL | Estado | Acción |
|---|---|---|---|
| Nómos | https://nomos.stevenvallejo.com | 🔴 500 (Neon quota exhausted) | issue abierta https://github.com/stevenvo780/scrapekit/issues/1 con logs completos |
| Koinonía | https://koinonia.stevenvallejo.com | ✅ 200 OK (medido con `curl -v`) | mi medición anterior de "timeout" fue fluke; verificar visualmente |
| Elenxos apex | https://elenxos.com (sin www) | 🔴 000 NXDOMAIN | añadir www→Vercel o apex A record |

## Metadata que ya está medida (la mayoría de los productos)
- HTTP status + tamaño
- Stack (Next.js / Vite / FastAPI) inferido de headers
- Idioma (atributo `lang` y meta)
- og:image y og:description
- theme-color (paleta de marca)
- PWA manifest (cuando existe)
- Repo GitHub + último push
- Vercel project + último deploy
- description meta tag (la de arriba viene de ahí)

## Lo que NO puedo medir sin permiso
- Tráfico real / visitantes
- Sign-ups / MRR / revenue
- Uptime histórico (sólo estado actual)
- Errores de runtime sin logs de Vercel

## Mejoras opcionales del catálogo (sin tocar productos)
- Capturar og:image de cada producto y mostrarlas en la página del catálogo
- Sincronizar este fichero con `catalogo.humanizar.tech` (alimentar la página desde aquí)
- Capturar screenshots para visual review
- Agregar favicons como íconos
- Health check automático cada 24h → alerta si alguno cae

## Lecciones de medición de este turno
- **elenxos.com apex vs www:** apex da 000 (NXDOMAIN), www da 200. Diferencia 1 carácter.
- **stevenvallejo.com redirect:** siempre con `-L`, sin él sale 307 con cuerpo "Redirecting..." (15 B).
- **curl HEAD vs GET:** Nómos con HEAD devolvía 405, con GET 500. Distintos códigos, misma app.
- **curl con timeout corto vs largo:** Koinonia con timeout 5s parecía timeout, con 12s es 200 OK.
- **`vercel inspect <project>` ≠ `vercel ls <project>`:** el primero pide el nombre EXACTO del deployment.
- **`vercel domains inspect` no muestra a qué proyecto pertenece** el dominio — hay que cruzar con `vercel ls` por URL.